...
enum class WeaponType { LASER, BULLET, TORPEDO, COUNT }; 
class Projectile {
public:
	Projectile(Vector2 pos, Vector2 vel, int dmg, WeaponType wt, bool explosive = false, float radius = 0.f) 
	{
	...
	float GetRadius() const {
		return (type == WeaponType::BULLET) ? 5.f :
		(type == WeaponType::TORPEDO)? 8.f : 2.f;
	}
	...
	float speed)
{
	...
	else if (wt == WeaponType::TORPEDO){ 
		return Projectile(pos, vel, 50, wt, true, 100.f);
	}
	else {
...
public:
	...
	float GetFireRate(WeaponType wt) const {
		...
		(wt == WeaponType::BULLET) ? fireRateBullet : fireRateTorpedo;
	}

	float GetSpacing(WeaponType wt) const {
		return (wt == WeaponType::LASER) ? spacingLaser : (wt == WeaponType::BULLET) ? spacingBullet : spacingTorpedo;
	}
...
protected:
	...
	float      spacingLaser;
	float      spacingBullet;
	float 	   fireRateTorpedo;
	float 	   spacingTorpedo;
};
	...
	void Run() {
		...
		while (!WindowShouldClose()) {
			...
			// Shooting
			{
				if (player->IsAlive() && IsKeyDown(KEY_SPACE)) {
					...
					while (shotTimer >= interval) {
						//if (currentWeapon == WeaponType::TORPEDO) { // only one torpedo at a time
           				 //	 bool exists =
        				//}
			...
			for (auto pit = projectiles.begin(); pit != projectiles.end();) {
				...
				for (auto ait = asteroids.begin(); ait != asteroids.end();) {
					...
					if (dist < (*pit).GetRadius() + (*ait)->GetRadius()) {
						if ((*pit).IsExplosive() == true) {
							float explosionRadius = (*pit).GetExplosionRadius();
							for (auto ait2 = asteroids.begin(); ait2 != asteroids.end(); ++ait2) {
								if (ait2 != ait) {
									float dist2 = Vector2Distance((*pit).GetPosition(), (*ait2)->GetPosition());
									if (dist2 < explosionRadius + (*ait2)->GetRadius()) {
										ait2 = asteroids.erase(ait2);
									}
								}
							}
						}
			...
			// Render everything
			{
				...
				if (currentWeapon == WeaponType::TORPEDO) weaponName = "TORPEDO";
